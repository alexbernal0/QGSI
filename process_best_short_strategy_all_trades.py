"""
Process SHORT signals from pre-split chunk files
Best SHORT Strategy: ATR Trailing Stop, Multiplier 1.5×
"""
...
from BEST_SHORT_STRATEGY_ATR_Trailing_Stop import backtest_short_trailing

def process_all_chunks():
    """Process all chunk files for SHORT signals."""
    
    print("=" * 80)
    print("PROCESSING BEST SHORT STRATEGY FROM CHUNKS")
    print("Parameters: ATR(30), Multiplier 1.5×")
    print("=" * 80)
    ...
    for chunk_num, chunk_file in enumerate(chunk_files, 1):
        ...
        for symbol in symbols_in_chunk:
            ...
            trades = backtest_short_trailing(
                df=symbol_df,
                symbol=symbol,
                atr_period=30,
                multiplier=1.5,
    ...
    # Save final parquet
    output_path = '/home/ubuntu/stage4_optimization/Best_Short_Strategy_ATR_Trailing_Trades.parquet'
    final_trades.to_parquet(output_path, index=False)
    file_size_mb = os.path.getsize(output_path) / (1024 * 1024)
    print(f"\n✓ Final parquet saved to: {output_path} ({file_size_mb:.1f} MB)")
    
    # Save summary to CSV
    summary_path = '/home/ubuntu/stage4_optimization/Best_Short_Strategy_Summary.csv'
